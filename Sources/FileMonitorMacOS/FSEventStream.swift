...
//		NSLog("   Event id: %llu, Event flags: 0x%x, Event path: %@", currentEventId, currentEventFlags, currentEventPath);
		...
		if currentEventFlags == kFSEventStreamEventFlagNone {
			...
			switch itemTypeFlags {
				...
				case FSEventStreamEventFlags(kFSEventStreamEventFlagItemIsDir):          itemType = .directory
				case FSEventStreamEventFlags(kFSEventStreamEventFlagItemIsSymlink):      itemType = .symboliclink
